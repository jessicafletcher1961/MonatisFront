...
- `AdminPortableBuilderPanel.tsx` affiche l'état du microservice `Micro_Service_make_exe`, vérifie la présence d'un JDK complet et permet de créer une image portable Windows. Si aucun JDK n'est présent, le premier build peut installer automatiquement un JDK portable local dans `Micro_Service_make_exe/tools/jdk`. L'utilisateur choisit le dossier du back à compiler, puis le dossier d'export. Le build se fait d'abord dans le dossier de travail du microservice ; l'utilisateur peut ensuite copier l'image vers le dossier d'export ou télécharger un ZIP. La case `Inclure la base actuelle` copie le dossier `data` du back choisi dans le portable seulement si le back local est arrêté. Cette section ne consomme pas le back métier et ne modifie pas ses sources.
