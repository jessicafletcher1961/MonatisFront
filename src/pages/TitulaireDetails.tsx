...
export default function TitulaireDetails() {
  const nav = useNavigate();
  const { nom } = useParams();

  const refQ = useQuery({
    queryKey: ["references", "titulaire", nom],
    queryFn: () => getReference("titulaire", nom || ""),
    enabled: !!nom
  });
  ...
  const comptesAssocies = useMemo(() => {
    ...
    const key = nom || "";
    return all
      .filter(c => (c.nomsTitulaires ?? []).includes(key))
      .slice()
  ...
  return (
    <div>
      <PageHeader
        title={nom ? `Titulaire · ${nom}` : "Titulaire"}
        subtitle="Détail du titulaire + comptes internes associés."
        right={
          <Button variant="secondary" onClick={() => nav("/titulaires")}>Retour à la liste</Button>
        }
      />
      ...
      {refQ.isLoading ? (
        ...
        <div className="text-sm text-blush-200">Impossible de charger ce titulaire.</div>
      ) : !refQ.data ? (
        <div className="text-sm text-white/60">Titulaire introuvable.</div>
      ) : (
            ...
            <Card className="lg:col-span-1">
              <CardHeader>
                <div>
                  ...
                  <CardDescription>Référence Titulaire (nom + libellé).</CardDescription>
                </div>
              </CardHeader>
            ...
            <Card className="lg:col-span-2">
              <CardHeader>
                <div>
                  ...
                  <CardDescription>
                    {comptesAssocies.length} compte(s) trouvé(s).
                  </CardDescription>
                </div>
              </CardHeader>
              ...
              {comptesQ.isLoading ? (
                ...
                <div className="p-4 text-sm text-white/60">Aucun compte associé.</div>
              ) : (
